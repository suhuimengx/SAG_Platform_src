...
//    }
    ...
    Ptr<Ipv4RoutingProtocol> tempRouter = _ipv4->GetRoutingProtocol();

	Ipv4Header tempHeader;
	tempHeader.SetDestination (gateway);
	Socket::SocketErrno errno_;
	Ptr<Ipv4Route> route;
	Ptr<NetDevice> oif = nullptr;
	route = tempRouter->RouteOutput (Ptr<Packet> (), tempHeader, oif, errno_);
...
Ipv4Address BgpRouting::LookupAsnGateway (Ipv4Address dst) {
	const libbgp::BgpRib4Entry *rslt = _rib->lookup(htonl(dst.Get()));
