...
void Bgp::StartApplication(void) {
	...
	// modify{
    Ptr<Ipv4ListRouting> routingList = GetNode()->GetObject<Ipv4>()->GetRoutingProtocol()->GetObject<Ipv4ListRouting>();
	uint32_t NRoutingProtocols = routingList->GetNRoutingProtocols();
	bool found = false;
	for(uint32_t i = 0; i < NRoutingProtocols; i++){
		int16_t priority;
		if(routingList->GetRoutingProtocol(i, priority)->GetInstanceTypeId() == TypeId::LookupByName ("ns3::BgpRouting")){
			_routing = routingList->GetRoutingProtocol(i, priority)->GetObject<BgpRouting>();
			found = true;
			break;
		}
	}
	if(!found){
		throw std::runtime_error ("BgpRouting not found in Routing List.");
	}

    _routing->SetRib(&_rib);

//    int16_t priority = 0;
//    uint32_t index = GetNode()->GetObject<Ipv4>()->GetRoutingProtocol()->GetObject<Ipv4ListRouting>()->GetNRoutingProtocols();
//    _routing = GetNode()->GetObject<Ipv4>()->GetRoutingProtocol()->GetObject<Ipv4ListRouting>()->GetRoutingProtocol(index-1,priority)->GetObject<BgpRouting>();
//    _routing->SetRib(&_rib);
    // }modify
...
// modify{
void Bgp::MakeRegularConnectPeer(double time) {
	...
	int64_t next_update_ns = time + m_dynamicStateUpdateIntervalNs + 2;

	if (next_update_ns < m_simulation_end_time_ns)	{
			Simulator::Schedule(NanoSeconds(m_dynamicStateUpdateIntervalNs),
					&Bgp::MakeRegularConnectPeer, this, next_update_ns);
	}

}

void Bgp::SetSimulation(int64_t simulation_end_time_ns, double dynamicStateUpdateIntervalNs) {
	m_simulation_end_time_ns = simulation_end_time_ns;
    m_dynamicStateUpdateIntervalNs = dynamicStateUpdateIntervalNs;
}

Ptr<Peer> Bgp::GetPeer(uint32_t i) const {
	return _peers.at(i);
}

size_t Bgp::GetNumPeer(void) const {
	return _peers.size();
}

void Bgp::DeletePeer(uint32_t i) {
	if(i >= 0 && i < _peers.size()) {
		_peers.erase(_peers.begin() + i);
	}
	else{
		std::cout << "failed to delete peer." << std::endl;
	}
}
// }modify
...
//    }
    ...
    // modify{
	if (peer_socket->GetObject<TcpSocketBase>()->Connect(InetSocketAddress(peer->peer_address, 179), peer->local_address) == -1) {
		NS_LOG_ERROR("failed to connect: " << strerror(errno));
		return false;
	}
	// }modify
...
/**
 * @brief Remove all routes from RIB.
 *
 * @param src_router_id Originating BGP speaker's ID in host bytes order.
 */
void Bgp::ClearRib4(uint32_t src_router_id) {
    //NS_ASSERT(!_running);
	//auto htonl_src_router_id = htonl(src_router_id);
	_rib.clear();
    //std::cout << " > Delete Route  gnd router_id:" << htonl_src_router_id << "(in network bytes order)" << std::endl;
}

/**
 * @brief Print number of routes in RIB.
 *
 * @param src_router_id Originating BGP speaker's ID in host bytes order.
 */
void Bgp::PrintNumRib4(uint32_t src_router_id) {
	auto htonl_src_router_id = htonl(src_router_id);
	libbgp::rib4_t rib4_t = _rib.get();
	std::cout << "gnd router_id:" << htonl_src_router_id << " RIB size:" << rib4_t.size() << std::endl;
}
 ...
 *
 * @return _uint_bgp_id Router ID in host bytes order.
 */
uint32_t Bgp::GetBgpId(void) const {
    //NS_ASSERT(!_running);
    uint32_t _uint_bgp_id = _bgp_id.Get();
    return _uint_bgp_id;
}
