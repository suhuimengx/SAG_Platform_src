...
void Ipv6L3Protocol::Send (Ptr<Packet> packet, Ipv6Address source, Ipv6Address destination, uint8_t protocol, Ptr<Ipv6Route> route)
{
  ...
  NS_LOG_LOGIC ("Packet to " << destination << " send on node " << m_node->GetId ());
  ...
  if (newRoute)
    {
      ...
      m_sendOutgoingTrace (hdr, packet, interface);
      NS_LOG_LOGIC ("Packet send send to " << destination << "at interface" << interface << "by new route");
          ...
          for (uint32_t i = 0; i < GetNAddresses (j); i++)
            {
              ...
              NS_LOG_LOGIC ("My Address " << addr << " not a match");
            }
        }
    }
  NS_LOG_LOGIC ("Receive Address " << hdr.GetDestination () << " not a local match");
