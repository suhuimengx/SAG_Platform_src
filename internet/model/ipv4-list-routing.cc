...
//			  rprotoIter++)
//	  {
//		if((*rprotoIter).second->GetInstanceTypeId() == TypeId::LookupByName ("ns3::BgpRouting")){
//			Ipv4Address asnGateway = (*rprotoIter).second->LookupAsnGateway();
//			if(asnGateway.IsAny()){
//				break;
//			}
//
...
//				if ((*rprotoIter).second->RouteInput (p, header, idev, ucb, mcb, downstreamLcb, ecb))
//				{
//				  //std::cout<<(*rprotoIter).second->GetInstanceTypeId()<<std::endl;
//				  NS_LOG_LOGIC ("Route found to forward packet in protocol " << (*rprotoIter).second->GetInstanceTypeId ().GetName ());
//				  return true;
//				}
//			}
