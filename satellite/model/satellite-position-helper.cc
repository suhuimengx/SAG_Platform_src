...
Vector3D
SatellitePositionHelper::GetPositionInECI (void) const{
	if (!m_sat)
		return Vector3D (0,0,0);
	...
	return m_sat->GetPositionInECI (cur);
}
...
Vector3D
SatellitePositionHelper::GetVelocityInECI (void) const
{
  if (!m_sat)
    return Vector3D (0,0,0);
  ...
  return m_sat->GetVelocityInECI (cur);
}
