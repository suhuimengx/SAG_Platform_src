...
//  std::string currentDir_s(currentDir);
  std::string mpi_readin_dir_guid = MpiInterface::mpireadin;
  m_minDelayFilePrefix = "./data/"+mpi_readin_dir_guid+"/config_topology";

	json j;
	std::ifstream config_file;
	config_file.open("./data/"+mpi_readin_dir_guid+"/basic_attribute.json");
	NS_ABORT_MSG_UNLESS(config_file.is_open(), "File basic_attribute.json could not be opened");
	config_file >> j;
	m_dynamic_state_update_interval_ns = j["basic_simulation_set"]["dynamic_state_update_interval_ns"];
	config_file.close();
	//std::cout<<"> Distributed simulator impl readin dir: "<<currentDir_s<<std::endl;
}
  ...
  if (MpiInterface::GetSize () <= 1)
  ...
  else
    {
	  if(Simulator::Now() == Time(0)){
			std::ifstream config_file;
			std::string line;
			std::uint64_t lookAhead = 1000000;
			config_file.open(m_minDelayFilePrefix+"/system_" + std::to_string(MpiInterface::GetSystemId ())+"_mpi_lookahead.txt");
			if (config_file) {
				std::getline(config_file, line);
				// Split on =
				//std::string res = split_string(line, " ", 1);

				//std::cout<<line<<std::endl;
				lookAhead = std::stoi(line);

			}
			else{
				throw std::runtime_error("Please change the Absolute Path 'config_file' for mpi lookahead file!");
			}
			config_file.close();
		  m_lookAhead = Time(lookAhead);
...
//			}
  ...
  //std::cout<<Simulator::Now().GetSeconds()<<"  "<<m_grantedTime.GetSeconds()<<"   "<<m_lookAhead.GetSeconds()<<std::endl;
}
...
//	  std::cout<<next.key.m_ts<<"  "<<m_currentTs<<std::endl;
//	  std::cout<<next.key.m_uid<<"  "<<next.key.m_context<<std::endl;
//  }
  m_unscheduledEvents--;
  m_eventCount++;
...
DistributedSimulatorImpl::Run (void)
{
  ...
  if(Simulator::Now().GetNanoSeconds() == 0){
	  nextUpdateTime = m_dynamic_state_update_interval_ns + 1;
  }
  else{
	  nextUpdateTime = m_dynamic_state_update_interval_ns;
  }
  while (!m_globalFinished)
    {
      Time nextTime = Next ();
      if (nextTime > NanoSeconds(nextUpdateTime))
	  {
    	  //std::cout<<nextTime<<"  "<<nextUpdateTime<<std::endl;
		  CalculateLookAhead ();
		  nextUpdateTime = nextUpdateTime + m_dynamic_state_update_interval_ns;
	  }
