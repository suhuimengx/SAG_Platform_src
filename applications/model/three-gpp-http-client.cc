...
void
ThreeGppHttpClient::ClientMainObjectReceived (Ptr<const ThreeGppHttpClient> client, Ptr<const Packet> packet){
...
void
ThreeGppHttpClient::ClientEmbeddedObjectReceived (Ptr<const ThreeGppHttpClient> client, Ptr<const Packet> packet){
	Ptr<Packet> p = packet->Copy ();
	ThreeGppHttpHeader header;
	p->RemoveHeader (header);
	if (header.GetContentLength () == p->GetSize ()
...
void
ThreeGppHttpClient::CompleteObjectRxRTT(const Time & t, const Address &address){
	  NS_LOG_INFO ("Client received a complete object with rtt "<<t.GetSeconds()<<" s"<<" from " << address);
	  m_rtt.push_back(t.GetSeconds());
}

void
ThreeGppHttpClient::CompleteObjectRxDelay(const Time & t, const Address &address){
