...
std::vector<Prefix4> BgpRib4::discard2(uint32_t src_router_id) {
    std::lock_guard<std::recursive_mutex> lock(mutex);
    std::vector<Prefix4> dropped_routes;

    for (rib4_t::iterator it = rib.begin(); it != rib.end();) {
        if (it->second.src_router_id == src_router_id) {
            ...
            LIBBGP_LOG(logger, DEBUG) {
                ...
                inet_ntop(AF_INET, &src_router_id, src_router_id_str, INET_ADDRSTRLEN);
                inet_ntop(AF_INET, &prefix, prefix_str, INET_ADDRSTRLEN);
                logger->log(DEBUG, "BgpRib4::deleteRoutes: dropped route (%s), scope %s, route %s/%d\n", "silent", src_router_id_str, prefix_str, it->second.route.getLength());
            }
