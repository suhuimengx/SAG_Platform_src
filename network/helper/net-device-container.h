...
public:
  ...
  /**
   * \brief Get the Ptr<NetDevice> stored in this container with key if exits
   *
   * \param other The NetDeviceContainer to append.
   */
  Ptr<NetDevice> GetWithKey (std::string key) const;
  ...
  /**
   * \brief Append a single Ptr<NetDevice> to this container and add its key for search.
   *
   * \param device The Ptr<NetDevice> to append.
   * \param key The key for the device defined by user with a string type.
   */
  void AddWithKey (Ptr<NetDevice> device, std::string key);

  /**
   * \brief Erase a single Ptr<NetDevice> in this container and del its key for search.
   *
   * \param key The key for the device defined by user with a string type.
   */
  void DelWithKey (std::string key);
