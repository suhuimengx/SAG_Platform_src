...
Ptr<NetDevice>
NetDeviceContainer::GetWithKey (std::string key) const
{
  auto iter = m_keys.find(key);
  if (iter != m_keys.end())
  {
    return m_devices[iter->second];
  }
  return nullptr;
}
...
void
NetDeviceContainer::AddWithKey (Ptr<NetDevice> device, std::string key)
{
  m_devices.push_back (device);
  if(m_keys.find(key)!=m_keys.end())
  {
    //std::cout << "key of " << key << "have used by" <<   << std::endl;
  }
  m_keys[key]=GetN()-1;
}
void
NetDeviceContainer::DelWithKey (std::string key)
{
  auto iter = m_keys.find(key);
  if(iter != m_keys.end())
  {
    auto itor = m_devices.begin() + iter->second;
