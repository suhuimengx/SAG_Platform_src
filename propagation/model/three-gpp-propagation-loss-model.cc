...
calculateElevationAngle(const Vector3D& a, const Vector3D& b) {
 ...
 double dz = b.z - a.z;
 // 计算地面站位置在地球表面的投影
 double distanceA = std::sqrt(a.x * a.x + a.y * a.y + a.z * a.z);
 double distanceToB = std::sqrt(dx * dx + dy * dy + dz * dz);
 // 计算向量之间的夹角（余弦定理）
 double cosTheta = (a.x * dx + a.y * dy + a.z * dz) / (distanceA * distanceToB);
 // 计算仰角
 double elevationAngle = std::acos(cosTheta) * 180.0 / M_PI;
 // 仰角是90度减去地心角
                                                      ...
                                                      Ptr<MobilityModel> b) const
{
    ...
    elev_angle = calculateElevationAngle(a->GetPosition(), b->GetPosition());
