...
Ipv4Address
BpEndpointId::GetAddress() const
{
  NS_LOG_FUNCTION(this);
  std::string ssp = Ssp();
  size_t colon_pos = ssp.find(':');
  if (colon_pos != std::string::npos)
  {
    std::string address = ssp.substr(0, colon_pos);
...
BpEndpointId::GetPort() const
{
  ...
  if (colon_pos != std::string::npos)
  {
    std::string port = ssp.substr(colon_pos + 1);
